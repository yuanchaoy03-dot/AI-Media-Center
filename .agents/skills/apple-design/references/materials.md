...
- **Where the approved design calls for translucency, build floating surfaces as layers** (`backdrop-filter: blur()` + a semi-transparent background) with content scrolling underneath — not opaque bars that consume a fixed strip.
...
- **Keep blur stable during motion.** Prefer transform/opacity transitions. Do not animate backdrop blur against project rules or add scale to an established no-scale component.
