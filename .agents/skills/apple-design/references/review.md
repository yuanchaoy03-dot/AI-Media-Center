...
Review only requested aspects. Sound and haptics apply only when explicitly part of the product task; they are not desktop defaults.

## Design foundations — the eight principles

The original skill grouped its design review guidance into eight principles. Use these as optional review lenses, not a mandatory checklist or permission to redesign:
...
- **Direct, specific labels beat safe generic ones.** Review whether labels describe destinations. Preserve approved navigation labels and order; renaming existing entries requires a design-change task.
