# Direct manipulation and gesture physics
...
Load only for an actual drag/momentum task or requested future touch design. Mobile sheets, swipe thresholds and touch hit padding do not expand the current desktop scope.
...
```
relativeVelocity = abs(targetValue - currentValue) > epsilon
  ? gestureVelocity / (targetValue - currentValue)
  : 0 // already at target; avoid division by zero
```
...
Exponential-decay projection example retained from the original skill:
...
animateSpringTo(target, { velocity: releaseVelocity }); // then hand off release velocity
```

Use projection only for an actual momentum interaction; do not replace native page scrolling or add snap behavior to a free-scrolling shelf.
