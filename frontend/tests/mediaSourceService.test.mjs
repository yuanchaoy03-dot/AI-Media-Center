...
test('DirectoryBrowser actual draft: disabled roots selected, navigation independent, confirmation before replacement', async () => {
  ...
  assert.equal(ui.selectionState(root.path).text, '扫描已暂停')
  assert.equal(ui.selectionState('/Movies').icon, 'plus')
  assert.equal(ui.selectionState('/Movies').text, '已选其中 3 个')
  ui.select('/Movies')
  ...
  assert.equal(ui.selectionState('/Movies').text, '')
  assert.equal(ui.selectionState('/Movies').icon, 'check-circle')
