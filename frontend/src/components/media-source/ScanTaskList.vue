...
<template>
  <div class="scan-list">
    <article v-for="task in tasks" :key="task.id" class="scan-item">
      ...
      <span class="scan-copy"><strong>{{ sources.find(source => source.id === task.sourceId)?.name }}</strong><span>{{ statusLabels[task.status] }} · {{ task.status === 'running' ? '仅扫描已启用的目录' : `新增 ${task.recognizedMovieIds.length} 部 · 待确认 ${task.pendingCount} 个 · 需要注意 ${task.attentionCount} 个` }}</span></span>
