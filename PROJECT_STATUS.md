...
- 登录 / 注册视觉更新：共用 AuthForm 新增本地开发素材的私人电影收藏背景与统一深色磨砂玻璃认证窗口；保持独立品牌/footer、窄窗口适配、短窗口自然滚动及减少动效/透明度和无滤镜回退。仍为 UI Preview，尚未接入 Spring Boot Authentication。此次类型检查与构建通过；浏览器连接无可用实例，尚未完成新视觉截图、交互和窗口尺寸实测，以下旧验证记录不代表新视觉验收。
