...
- 最近主页数据收尾：最近添加及未看从公共 library-mock 派生单片预览，移除主页手写重复列表；按 added 倒序，固定最多 8 部，主页未看仅取 unwatched。复用现有溢出检测控制两条内容架箭头显隐，完整资料库不受上限影响。六档宽度、空数据/超限、共享数据修改同步、窗口缩放及资料库截图回归检查通过。
