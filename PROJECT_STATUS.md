...
- 最近主页 Shelf 交互优化：四类横向内容架取消右上角箭头及专属控制逻辑，统一支持纵向鼠标滚轮横向浏览；仅在对应方向仍可滚动时拦截，首尾边界及无溢出时放行页面滚动。映射滚轮期间暂缓吸附，离开或切回原生横向、触摸、键盘操作时恢复 proximity；六档响应式、桌面边界释放、触摸滑动与卡片/导航回归检查通过，保持 file:// Mock，无新增依赖或网络请求。
