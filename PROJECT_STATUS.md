...
- 当前前端切片状态：片库、筛选排序、加载状态、合集详情、全局搜索和电影详情已迁移；详情入口已接通。媒体来源、主页及其他未完整迁移页面仍不能按 HTML 原型完成度计为 Vue 完成。
...
- 登录 / 注册视觉更新：共用 AuthForm 新增本地开发素材的私人电影收藏背景与统一深色磨砂玻璃认证窗口；本轮小幅提高背景亮度、降低窗口不透明度并调整磨砂与边缘高光，参数见 DESIGN 第31节，collage排列及表单保持不变。保持独立品牌/footer、窄窗口适配、短窗口自然滚动及减少动效/透明度和无滤镜回退。仍为 UI Preview，尚未接入 Spring Boot Authentication。此次类型检查与构建通过；浏览器连接无可用实例，尚未完成新视觉截图、交互和窗口尺寸实测，以下旧验证记录不代表新视觉验收。
