...
影片文件夹选择沿用深色桌面 Dialog：以“当前文件夹 / 路径 / 选择整个文件夹及里面的内容”明确整体选择，来源顶层标为“整个来源”；下方“里面的文件夹”提示可只选一部分。目录主体进入下一级，右侧独立36×36px控件选择，图标20px；未选用 plus，已选用 check-circle 和现有 focus 色。随上级一起选中的文件夹用较弱的 folder 图标及具名状态说明，仅禁用选择控件；含已选文件夹时显示数量，不使用半选框。底部显示已选文件夹总数与取消/保存，不使用 chips。选择整个文件夹替换已单独选择的内部文件夹时，确认 Dialog 用文件夹名称说明变化并保留路径列表，不表达云端删除。弹窗及目录列表保留桌面短窗口滚动。
