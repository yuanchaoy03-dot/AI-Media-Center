...
影片文件夹选择沿用深色桌面 Dialog：显示“当前文件夹”及路径，来源顶层标为“整个来源”；下方列出“里面的文件夹”。目录主体进入下一级，caret-right 表示可进入；右侧独立36×36px控件选择，图标20px。未选用 plus，直接选中用现有 focus 色的 check-circle；随上级包含用淡化的 check-circle，仅禁用选择控件，原因放在 title / aria-label，不常驻解释。部分选择时显示“已选其中 N 个”，不使用半选框；已暂停扫描的直接选中根保留短状态文字。底部显示已选文件夹总数与取消/保存，不使用 chips。选择整个文件夹替换已单独选择的内部文件夹时，确认 Dialog 用文件夹名称说明变化并保留路径列表，不表达云端删除。弹窗及目录列表保留桌面短窗口滚动。
...
认证入口按本次确认使用独立深色磨砂窗口，作为第11节表单材质的限定例外：背景 `rgba(18,18,20,.68)`、blur 22px、saturate 125%、1px白色13%边框、20px圆角，阴影 `0 24px 64px rgba(0,0,0,.32)` 与顶部10%内高光，左上145deg表面渐变高光由6%经30%位置的1.5%衰减至55%位置的透明；认证窗口不随系统 `prefers-reduced-transparency` 自动降级，仅无滤镜或显式 `html[data-transparency='reduced']` 时使用 `rgba(20,20,22,.94)`。桌面内边距36px/34px，≤640px时28px/22px，窗口宽度不超过视口减32px。输入底色白色5.5%、边框9%，聚焦底色7.5%、边框22%，保留焦点环和错误边框。窗口260ms淡入并上移8px，减少动效时关闭。
