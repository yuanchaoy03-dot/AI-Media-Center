...
扫描目录管理沿用深色桌面 Dialog：目录主体进入下一级，右侧独立36×36px控件选择扫描根，图标20px；未选用 plus，已选用 check-circle 和现有 focus 色。被祖先包含时用较弱的 folder 图标并显示“已包含于路径”，仅禁用选择控件；包含已选后代时显示数量，不使用半选框。当前路径共用相同状态，底部显示总数与取消/保存，不再使用“本次已选”chips。父目录替换使用现有确认 Dialog 列出下级路径，不表达云端删除。弹窗及目录列表保留桌面短窗口滚动。
