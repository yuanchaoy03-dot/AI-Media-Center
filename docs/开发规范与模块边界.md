...
- MediaSource连接管理、连接测试、WebDAV目录浏览、MediaScanRoot扫描范围管理和ScanTask扫描任务管理。
...
1. **单一业务事实源**：用户、Movie、MediaResource、MediaSource、MediaScanRoot、收藏、观看状态和播放记录的权威状态只在Spring Boot/MySQL中维护。
...
11. **用户隔离贯穿全链路**：MediaSource、MediaScanRoot、MediaResource可见范围、个人片库、收藏、观看状态、播放历史、AI/RAG数据和会话均由可信CurrentUser限定；新用户不得继承公共或他人片库。
...
| `media` | MediaSource、MediaScanRoot、WebDAV目录浏览、扫描范围管理、MediaResource、ScanTask、WebDavMediaSourceAdapter、确定性命名解析、Edition/Cut、ffprobe探测、ResourceLocator、资源状态 | Movie事实生成、个人观看状态、真正视频播放 |
...
- 表达一个清晰业务用例，例如“确认影片匹配”“扫描已配置MediaScanRoot”“收藏影片”“生成播放描述”。
...
- TMDB Base URL、Token、文字元数据language、region、fallback、配额和超时全部通过配置管理。
...
- 确认TMDB Movie ID后必须先查询公共Movie。已存在时只复用并关联当前MediaResource，不重复获取完整Details、重新选择Poster/Backdrop、下载已有图片或上传OSS，不为用户生成不同图片。
- 公共Movie不存在时，由一个统一Application Service编排TMDB规范化事实、直接使用TMDB Adapter规范化返回的Movie Details默认`poster_path`/`backdrop_path`、图片下载、OSS上传、Movie创建与MediaResource关联。概念名可用`MovieIngestionService`或`MovieScrapeService`，正式命名按代码实际确定；普通扫描、未来管理员预建和未来批量预热不得各写一套逻辑；后两者仍为非P0，均采用相同Details默认主图，不增加管理员手动挑海报。
- P0不实现图片语言筛选算法；`TmdbMovieMetadataProvider`返回Movie Details默认`poster_path`/`backdrop_path`的规范化来源信息，Application Service直接使用这两个来源完成下载与OSS持久化。每个Movie只维护一个主Poster和一个主Backdrop；不实现用户级选图、锁图、多候选图库或图片运营后台。
- 未有新需求时，禁止增加中文Poster查询、`/images`全量图片抓取、图片语言排序、基于`vote_average`/`vote_count`的图片评分算法、Poster候选选择器或管理员图片运营功能。文字元数据仍优先中文片名和简介，保留原始片名，中文缺失时允许TMDB其他可用语言回退；不得把该回退用于另选图片。
- `poster_path`缺失不阻止Movie建档，前端使用缺省海报占位；`backdrop_path`缺失允许为空并由前端合理降级，Poster与Backdrop不得互相替代。
- 图片来源去重以`provider + source_path`为主，稳定来源标识生成稳定OSS Object Key。同一TMDB图片重复遇到时必须幂等，不做SHA-256、MD5、感知哈希等内容级去重或多Provider内容归并。
- 图片二进制只存OSS，MySQL只保存TMDB来源追踪与OSS Object Key/稳定定位。Vue正式日常浏览只使用OSS/CDN定位，不直接引用TMDB图片CDN。
- 普通扫描（含第二个用户扫描和新MediaResource关联已有Movie）、登录、浏览、收藏、播放、AI搜索、RAG和Agent不得自动替换已经持久化的公共图片；只有显式重新刮削、刷新元数据或重新同步影片才允许更新，更新时仍读取Movie Details默认`poster_path`/`backdrop_path`，路径变化时可更新OSS和公共Movie图片定位，并使所有关联用户看到同一新图片。
...
- MediaSource必须表达当前用户保存的一套WebDAV连接配置；MediaScanRoot必须表达用户在该来源中明确选择、允许递归扫描的目录根，来源允许0~N个扫描根。创建MediaSource不得自动触发扫描，更不得整盘扫描或默认将`/`加入扫描根。
- WebDAV目录浏览必须作为独立只读能力，允许用户随时逐层浏览来源当前可见的完整目录结构；不得因为展开目录就创建MediaResource、执行TMDB刮削或ffprobe、建立个人片库关系或启动扫描。浏览只返回展示所需的直接子目录/文件基础元信息；目录层级及是否为扫描根由对应业务能力提供。
- 用户必须能够后续新增或移除MediaScanRoot，无需重建MediaSource；管理扫描根不自动启动扫描。移除或停用扫描根不得简单等价为删除公共Movie，已有资源和片库关系生命周期留待后续数据设计确认。
- ScanTask必须以当前用户本人已启用MediaSource下已配置、已启用的MediaScanRoot为边界；扫描Application Service负责加载扫描根、调用Adapter递归发现文件、创建或更新MediaResource，再进入识别 / TMDB / ffprobe流程。没有启用扫描根时不得产生实际扫描结果；缺失资源判定只能覆盖本轮完整扫描的根范围。
- 不得仅凭前端提交sourceId、path或scanRootId认定用户有权限；必须根据可信CurrentUser复核MediaSource归属、MediaScanRoot所属来源及路径边界，普通业务不得通过前端传userId决定身份。
- MediaScanRoot是`media`模块的实际业务概念，不得默认退化成MediaSource上的一个逗号分隔path字符串、单一路径或JSON数组。本文不设计数据库表、列名或物理存储结构。
...
- 目录浏览与扫描必须都通过MediaSourceAdapter访问WebDAV；连接测试、直接子项浏览和扫描根内递归发现是不同能力，不要求固定Java方法名。Adapter负责协议访问、文件/目录区分、文件事实读取、状态检查和当前播放定位解析，返回统一事实与错误类别，不直接创建MediaResource。
- WebDAV密码、Token、认证Header等凭据只在Spring Boot / Adapter基础设施边界使用，不得返回Vue或进入FastAPI、AI上下文、LLM、向量存储、OperationRecord、普通业务日志或跨模块DTO。
...
- MediaSource连接配置、连接测试、只读目录浏览、MediaScanRoot管理与根内扫描。
- TMDB影片识别、刮削和人工确认。
- ffprobe媒体探测及失败降级。
...
- TMDB Base URL、Token、文字元数据language、region和fallback参数。
- WebDAV连接的服务地址、认证信息和兼容参数；扫描范围单独由当前用户在来源下配置的MediaScanRoot表达，不合并为连接的唯一路径属性。
...
- 媒体来源切片必须测试：创建连接不自动扫描且不默认添加`/`；0个扫描根仍可测试/浏览但无扫描结果；浏览无MediaResource / 片库 / TMDB / ffprobe副作用；多个扫描根及后续增删无需重建连接；扫描与缺失判定不越出本轮有效范围；篡改sourceId、path、scanRootId不能访问他人来源或扫描根。
...
以下约束适用于涉及相应模块与能力的实现；无需为局部任务加载全部工程规范：

1. 功能所属业务模块与上游需求应明确；范围判断按对应正式基线。
...
12. 新用户注册后只能得到空片库；MediaSource、MediaResource和个人片库关系必须来自该用户随后添加WebDAV连接、配置MediaScanRoot并主动扫描的流程。公共Movie元数据可以预先存在和复用，但不得因此给用户添加资源或片库关系。
...
18. 上下文按 [AGENTS.md](../AGENTS.md) 的 Context Routing 选择；可以先读相关源码，不设固定阅读顺序。
19. 完成任务后按实际影响更新文档，不因单个按钮或组件改动重写需求/架构文档；当前进度写入`PROJECT_STATUS.md`，接口变化写入`docs/API.md`。

若请求与正式基线存在未解决的产品或架构冲突，指出具体事实；仅在缺少必要决策时请求确认。用户已明确授权的变更继续执行并更新对应文档，不因旧流程条款重复等待确认，不通过隐式代码改变架构。
...
21. **新注册用户的片库如何初始化？**  
   默认为空，不关联MediaSource、MediaResource、其他用户资源或个人片库关系；公共Movie元数据可以存在，但只有该用户添加自己的WebDAV连接、配置MediaScanRoot并主动扫描、建立资源关系后才可见。
...
33. **TMDB图片怎样进入正式页面？**  
   Spring Boot直接采用TMDB Movie Details默认`poster_path`/`backdrop_path`作为主Poster/Backdrop，按`provider + source_path`生成稳定Object Key并上传OSS，MySQL保存来源追踪与稳定定位，Vue日常从OSS/CDN加载。

---

## 38. 项目上下文与动态文档治理

文档职责与按需入口见 [docs/README.md](README.md)，Agent 行为与 Context Routing 统一由 [AGENTS.md](../AGENTS.md)维护。本文保留正式模块与工程规范，不复制 Agent 阅读流程。

2026-09-15 治理依据：[OpenAI Skills 与 Prompt 审计文章](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra)及 [GPT-6 Astra Model Guidance / Prompting best practices](https://developers.openai.com/api/docs/guides/latest-model)。引用用于追溯本次治理，不是每次任务的阅读依赖。

`PROJECT_STATUS.md`用于当前实现状态、最近完成、当前切片、下一步与已知问题；历史记录应与现状区分。`API.md`随真实页面/用例维护契约，以自身状态标识区分设计与实现。二者都不是所有任务的默认必读项，状态摘要也不替代稳定基线。
...
禁止每改一个按钮就重写需求文档，也禁止每增加一个Vue组件就重生成架构文档。稳定基线只在事实变化时修改，动态API与状态文档按切片及时更新。完成与验证尺度遵守 [AGENTS.md](../AGENTS.md)；只检查当前变更涉及的能力，不因纯文档修改运行无关代码测试。
