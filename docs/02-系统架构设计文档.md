...
> 用户注册/登录 → 默认空片库 → 添加WebDAV连接配置 → 测试连接 → 建立MediaSource → 浏览目录 → 配置一个或多个MediaScanRoot → 用户主动启动ScanTask → 根内文件发现与识别 → TMDB刮削 → MediaResource入库并关联Movie → 建立CurrentUser独立片库 → 搜索/收藏/观看记录 → AI搜索、推荐、RAG、Agent → 获取当前PlaybackLocator → 浏览器通过`mpv://`调用用户Windows本机mpv
...
flowchart TB
    ...
    System -->|"目录浏览、扫描根内发现、状态检查、解析播放定位"| Sources
...
3. 管理Movie、MediaResource、MediaSource和MediaScanRoot等核心领域对象。
4. 管理收藏、用户标签、AI标签状态、观看状态和播放启动记录。
5. 编排MediaScanRoot范围内的扫描、Edition Label确定性提取、文件名解析、TMDB候选匹配、置信度计算和人工修正。
...
18. 在TMDB Movie ID确认后优先查询并复用公共Movie；缺失时通过统一公共Movie建档用例获取规范化事实、采用Movie Details默认主图、下载图片、上传OSS、保存稳定定位并创建Movie。
...
浏览器JWT只证明“当前是谁”，不能证明可以访问任意对象。MediaSource、MediaScanRoot、MediaResource、Favorite、PersonalState、History和AI/Playback普通入口仍须根据CurrentUser执行用户范围与归属校验；OperationRecord普通入口也只返回本人记录，只有独立Admin审计入口在验证`ADMIN`后可查询系统级脱敏记录。Controller不得信任前端传入的`userId`。
...
2. **浏览目录（listDirectory / browseDirectory）**：访问当前用户拥有的MediaSource，列出指定目录的直接子项，区分directory / file并返回基础展示元信息；用户可逐层浏览该连接当前可见的完整目录结构，不受已选MediaScanRoot限制。浏览不创建MediaResource，也不触发扫描、TMDB、ffprobe或个人片库关系。
3. **扫描发现（scan / discoverResources）**：只在扫描Application Service传入的已配置、已启用MediaScanRoot内递归发现媒体文件，可采用分页或流式枚举；不得扩大到未选择的其他目录。
4. **获取文件信息**：返回文件名、稳定资源标识、相对位置、ResourceLocator、大小、修改时间，以及来源支持时的稳定资源ID。
5. **检查资源状态**：判断服务端是否仍可定位资源。
6. **解析播放定位**：将稳定资源定位转换为当前客户端可能使用的播放描述。

连接测试（testConnection）、目录浏览与扫描发现是三类独立职责；以上英文仅表达能力，不规定Java方法名或接口签名。来源应用服务先根据可信CurrentUser校验MediaSource归属；扫描Application Service再加载并校验该来源的MediaScanRoot、调用Adapter发现资源、创建或更新MediaResource，后续编排识别 / TMDB / ffprobe及个人片库关联。Adapter不负责业务入库。

来源接入流程为：添加WebDAV连接配置 → 测试连接 → 建立MediaSource → 浏览目录 → 配置一个或多个MediaScanRoot → 用户主动启动ScanTask。创建连接或浏览目录均不自动扫描，不默认添加`/`；0个扫描根或没有启用的扫描根时仍可测试、浏览，但不能产生实际扫描结果。

适配器返回统一结果和错误分类，例如认证失败、来源不可达、路径不存在、资源不存在、暂时超时和不可直接播放。核心业务不解析各来源特有异常文本。

### 12.3 适配器边界

- 适配器负责与来源通信，不负责创建Movie或MediaResource；目录浏览和扫描发现都经MediaSourceAdapter访问WebDAV，Application Service不得散落HTTP / DAV XML / Depth Header等协议代码。
- 适配器返回文件事实，不调用LLM。
...
## 15. Movie / MediaResource / MediaSource / MediaScanRoot领域关系
...
classDiagram
    ...
    MediaSource "1" --> "0..*" MediaScanRoot : 配置扫描边界
...
- **MediaSource**表示“当前用户保存的一套完整WebDAV连接配置”，表达来源身份、所属用户、连接配置、启用状态和连接状态，不绑定唯一扫描目录。
- **MediaScanRoot**表示“当前用户在某个MediaSource中明确选择、允许递归扫描的目录根”，表达所属来源、目录路径、可选展示名称和启用状态。MediaSource与MediaScanRoot为1:N关系，允许0~N个扫描根；用户可后续新增或移除扫描根，无需重建连接。MediaScanRoot与MediaSource、MediaResource、ScanTask均属于`media`模块。
- 上述为领域概念，不定义数据库表或列名；不得默认将MediaScanRoot退化为MediaSource上的单一路径、逗号分隔字符串或JSON路径数组。根目录重叠、任务执行期间配置变更以及移除/停用后的资源与片库关系生命周期由后续数据和扫描设计确认，但不得越出用户明确选择的扫描边界，也不等价于删除公共Movie。
...
- MediaScanRoot必须隶属于当前用户拥有的MediaSource；ScanTask只处理该来源当前已配置、已启用的MediaScanRoot，不扫描未选目录。
...
- MediaScanRoot同样按CurrentUser隔离。前端提交sourceId、path或scanRootId不证明权限；Spring Boot必须验证来源属于CurrentUser、扫描根属于该来源，并校验路径属于该连接可见范围，扫描时进一步受已配置扫描根约束。普通业务不得以传入userId决定身份。
- 目录浏览只返回展示所需的目录/文件元信息，WebDAV密码、Token、认证Header只在Spring Boot / Adapter边界内使用，不返回Vue、不进入AI上下文、OperationRecord或普通业务日志。
...
```mermaid
flowchart TD
    Start["用户主动请求ScanTask"] --> Roots["CurrentUser校验来源归属与启用状态；加载并校验MediaScanRoot"]
    Roots -->|"无启用扫描根"| NoRoots["提示先配置扫描根；不发现或创建资源"]
    Roots -->|"有启用扫描根"| Test["测试MediaSource连接"]
    Test -->|"失败"| Fail["任务失败；保留历史资源状态"]
    Test -->|"成功"| Enumerate["Adapter仅在已启用MediaScanRoot内递归枚举文件"]
    ...
    Lookup -->|"不存在"| Details["调用TMDB Movie Details获取P0事实与默认图片路径"]
    Details --> Select["采用Movie Details默认poster_path / backdrop_path"]
    ...
    Continue -->|"否且扫描完整"| Missing["仅在本轮完整扫描根范围内标记未发现资源为缺失"]
...
- 只有本轮对应MediaScanRoot的扫描成功完整结束后，才可将该实际扫描范围内未发现的历史资源标记为缺失；未扫描、已移除或已停用的扫描根不能据此判定资源缺失，其生命周期留待后续数据设计确认。
...
- 返回Movie Details默认`poster_path`的规范化来源信息，作为唯一主Poster；缺失不阻止建档，前端使用缺省海报占位。
- 返回Movie Details默认`backdrop_path`的规范化来源信息，作为唯一主Backdrop；缺失时允许为空，前端提供合理降级。
...
文字元数据的中文`language`、`region`、fallback及TMDB配额策略由TMDB Provider详细设计锁定；中文文字缺失时只使用TMDB可用语言降级，不能调用LLM翻译后冒充TMDB数据。Provider返回TMDB来源事实和Movie Details默认主图来源，公共Movie建档用例负责复用判断、采用Movie Details默认主图、图片持久化和Movie创建，避免把外部调用与业务一致性散落到多个入口。
...
2. 若Movie已经存在，立即复用已有记录，只建立当前用户MediaResource、个人片库关系及后续收藏/观看状态等个人数据；不得再次获取完整TMDB Details、重新选择Poster/Backdrop、下载已有图片或重复上传OSS，也不得为不同用户生成不同图片。
3. 若Movie不存在，调用TMDB Movie Details取得规范化事实以及默认`poster_path`、`backdrop_path`来源信息。
4. 直接将Movie Details默认`poster_path`和`backdrop_path`分别作为主Poster和主Backdrop，下载需要的图片并以稳定Object Key幂等上传自有OSS。
...
> 确认TMDB Movie ID → 按`tmdb_id`查询（已有Movie直接复用并关联）→ 仅缺失时由Spring Boot调用Movie Details取得影片事实 → 采用默认`poster_path`/`backdrop_path`作为主Poster/主Backdrop → Spring Boot下载图片 → 上传自有OSS → MySQL保存TMDB来源追踪与OSS稳定定位 → Vue日常从自有OSS/CDN加载

图片职责与规则如下：

- TMDB是影片元数据与图片来源；OSS是已选定图片的长期持久存储和日常分发来源。Vue正常浏览不得依赖`image.tmdb.org`，普通用户浏览已刮削片库时不再访问TMDB。
- 每个Movie在P0只维护一张主Poster和一张主Backdrop，所有关联用户共享同一套图片。直接采用TMDB Movie Details默认`poster_path`/`backdrop_path`，不执行中文、无语言、英文等图片语言优先级，也不按`vote_average`/`vote_count`或自定义评分排列图片；英文Poster不会触发另选中文图片。中文片名、中文简介等文字仍保持中文本地化优先及缺失回退，并保留原始片名。
- 产品希望尽量接近TMDB网站通常展示的主视觉或Most Popular / 最热门图片；P0的确定性规则仅为Movie Details默认路径，以沿用官方详情默认主图并避免额外图片排序逻辑。不保证`/movie/{id}/images`数组首项等于官网Most Popular，也不宣称复刻官网内部排序；未来实测存在明显差异时，精确适配仅作为非P0工程化增强。
...
- P0图片去重以`provider + source_path`作为来源身份，Spring Boot据此生成稳定OSS Object Key。同一TMDB图片重复遇到时不得产生多份对象；当前不做SHA-256、MD5、感知哈希等内容级去重或多Provider内容归并，多图片Provider与内容Hash留待真实需求出现后再设计。
- 普通扫描（含第二个用户扫描和新MediaResource关联已有Movie）、登录、浏览、收藏、播放、AI搜索、RAG和Agent不得比较TMDB默认图并自动覆盖现有OSS图片。只有显式“重新刮削”“刷新元数据”或“重新同步影片”操作才允许更新；更新后所有关联用户统一看到新图片。
- 显式刷新重新读取Movie Details默认`poster_path`/`backdrop_path`，路径变化时可更新OSS和公共Movie图片定位；仍应复用稳定来源标识和Object Key，避免每次刷新都向OSS写入重复对象；刷新时的覆盖、版本与失效清理细节留给后续数据库/对象存储详细设计。
- TMDB暂时不可达时，已有Movie继续从MySQL查询，已有Poster/Backdrop继续从OSS展示，只有新增电影、首次建档或显式刷新受影响。

未来管理员手动公共Movie预建档与热门Movie批量预热可以作为非P0轻量扩展，用于提前搜索/确认热门电影、完成公共Movie及图片持久化，提高普通用户首次扫描时的命中率并减少即时TMDB请求和图片下载压力。两者必须调用与普通用户首次扫描完全相同的公共Movie建档Application Service，只提前创建公共Movie及图片，不创建公共MediaResource、不替用户加片、不改变新用户空片库；同样采用Movie Details默认`poster_path`/`backdrop_path`，不提供管理员手动挑海报，也不得形成内容运营后台或第二套刮削逻辑。
...
flowchart LR
    subgraph MoviePath["影片识别与Movie事实链路：这是什么电影？"]
        MS["MediaSource"] --> Roots["已配置、已启用的MediaScanRoot"]
        Roots --> Task["用户主动启动ScanTask"]
        Task --> Discovery["MediaResource Discovery"]
        ...
        TMDB --> Ingestion["公共Movie建档与Details默认主图持久化"]
...
| --- | --- | --- | --- | --- |
| Movie / Person / Genre影片事实与展示图片 | TMDB官方API + 自有OSS | 通过TMDBMovieMetadataProvider获取事实与图片来源，经统一建档用例采用Movie Details默认主图、下载、上传OSS并保存稳定定位 | 只读消费、检索和解释 | LLM与ffprobe不得覆盖；每个Movie一套公共主图；Vue日常不直连TMDB图片CDN |
...
- MediaSource连接配置和非明文敏感配置引用，以及独立MediaScanRoot扫描范围配置；此处仅明确事实归属，不设计物理存储。
...
- 在TMDB Movie ID确认且公共Movie不存在时调用Movie Details获取01定义的完整P0影片事实；已有Movie仅在显式重新刮削/刷新/同步时重新获取。
- 优先请求中文本地化文字资料，并在中文缺失时按已配置的TMDB fallback规则降级。
- 返回Movie Details默认`poster_path`/`backdrop_path`的规范化来源信息，以及所有主要Director和cast order前20名演员；Spring Boot直接使用默认路径完成公共图片持久化。
- 不为主图选择调用`/movie/{id}/images`或向业务层暴露完整图片图库；Provider本身不把TMDB CDN URL当作Movie的长期日常图片定位。
...
TMDB供应商选择已经完成，不再列为待定。后续TMDB Provider详细设计只需锁定API凭据管理、文字元数据language、region、fallback、图片来源定位标准化、请求配额、重试、缓存边界和字段缺失策略；OSS Object Key与持久化由公共Movie建档边界处理。
...
- Controller不接受前端`userId`作为可信身份；Application Service负责MediaSource、MediaScanRoot、MediaResource及其他用户对象的归属和IDOR防护。
...
sequenceDiagram
    ...
    U->>V: 对已配置MediaScanRoot主动启动扫描
    V->>S: 请求创建ScanTask
    S->>S: CurrentUser校验来源归属、启用状态及扫描根归属
    S->>DB: 加载该来源已配置、已启用的MediaScanRoot
    break 无启用扫描根
        S-->>V: 提示先配置扫描根，不产生实际扫描结果
    end
    S-->>V: 返回任务已接受
    S->>A: 测试连接，仅在校验后的MediaScanRoot内递归发现文件
    loop 每个新增或变化资源
        ...
        alt 高置信度
            ...
            alt 公共Movie已存在
                ...
                S->>T: 调用Movie Details获取P0事实与默认图片路径
                T-->>S: Movie事实、默认poster_path、默认backdrop_path
                S->>S: 采用Movie Details默认poster_path / backdrop_path
...
显式重新刮削/刷新也复用同一公共Movie建档能力中的TMDB规范化、采用Movie Details默认主图和OSS持久化步骤；普通扫描命中已有Movie时不触发图片替换。未来管理员预建档或批量预热仍调用同一Application Service，不复制本时序中的刮削逻辑。
...
| ADR-037 | TMDB图片持久化与分发 | Spring Boot采用Movie Details默认主图并下载到自有OSS，Vue日常从OSS/CDN加载 | TMDB负责来源，OSS负责长期保存和稳定展示；TMDB故障不影响已有图片 |
...
| ADR-040 | 来源连接与扫描根分离 | MediaSource保存完整WebDAV连接，允许0~N个独立MediaScanRoot；目录浏览只读，ScanTask由用户主动发起且只处理已启用扫描根 | 添加连接不自动扫描、不默认选择`/`；支持后续增删目录而无需重建来源，归属由CurrentUser复核 |
...
- User、Movie、MediaResource、MediaSource、MediaScanRoot；User需要表达`USER/ADMIN`角色、账号允许/禁用状态、注册时间和可提供时的最近成功登录信息。具体字段、表名与索引由03决定，不建立企业级权限表体系。
...
- 片库和个人行为能力。
- 来源连接、连接测试、只读目录浏览、MediaScanRoot增删、以启用扫描根为边界的ScanTask和刮削能力；具体Endpoint / Request / Response随媒体来源Vue切片确认。
...
- TMDBMovieMetadataProvider的API凭据、文字元数据language/region/fallback、中文覆盖、Movie Details默认主图路径映射、配额、错误映射和字段缺失策略。
...
P0唯一影片事实源固定为TMDB，网络可达、API凭据、请求配额和中文资料缺失会直接影响新影片识别。应在开发早期使用代表性中外影片验证搜索、中文文字fallback、采用Movie Details默认主图、稳定Object Key、OSS上传与重复来源幂等，并确保TMDB故障只暂停新建档/显式刷新，不影响MySQL中已有Movie和OSS中已有图片。OSS故障应独立降级，不得回滚已有业务事实或把TMDB CDN临时变成正式长期链路。
...
28. **新用户首次进入系统看到什么？** 自主注册/登录后默认空片库：没有MediaSource、MediaResource、个人片库关系、其他用户资源或平台赠送的可播放影片。已有公共Movie元数据不会自动进入其片库，必须添加自己的WebDAV连接、配置MediaScanRoot并主动扫描后才建立资源关系。
...
39. **确认TMDB Movie ID后怎样建档？** 先按`tmdb_id`查询并复用；不存在时才取完整TMDB事实、采用Movie Details默认主图、下载并幂等上传OSS、创建Movie和关联资源。
40. **两个请求同时首次添加同一电影怎么办？** 以MySQL `UNIQUE(tmdb_id)`保证最终唯一；唯一冲突的一方重新查询并复用，未来Redis锁只能减少重复外部调用。
41. **影片图片日常从哪里读取？** TMDB提供原始来源，Spring Boot采用Movie Details默认主图并保存到自有OSS，MySQL保存来源追踪与稳定定位，Vue日常从OSS/CDN读取而不直连TMDB图片CDN。
